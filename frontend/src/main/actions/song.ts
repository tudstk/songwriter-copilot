...
const getSongFile = async (file: File): Promise<Song | null> => {
  if (file === null){
    return Promise.resolve(null)
  }
...
export const getSong =
  (rootStore: RootStore) => async (file: File) => {
    const song = await getSongFile(file)
    if (song === null) {
      console.error('Failed to get song from file');
      return;
    }
    console.log('Song received:', song); // Add this log to see if the song object is received correctly
